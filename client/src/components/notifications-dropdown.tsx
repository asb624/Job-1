...
export function NotificationsDropdown({ isMobile = false }: NotificationsDropdownProps) {
      ...
      {isLoading ? (
        ...
        <div>
          {notifications.map((notification: Notification) => (
            <DropdownMenuItem
              key={notification.id}
              className={`flex flex-col items-start p-4 cursor-pointer border-b border-teal-50 hover:bg-teal-50 transition-all duration-300 ease-in-out hover:pl-6 animate-slide-in-smooth ${
    ...
    return (
      <>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="p-0 overflow-hidden max-w-[95vw] w-[400px] rounded-xl origin-top-right animate-smooth-fade-in">
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent align="end" className="w-[350px] border-teal-100 shadow-lg rounded-xl overflow-hidden animate-smooth-fade-in">
